//===- SynthAnnotations.cpp - Synth Annotations -----------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the SynthAnnotations pass.  This pass processes FIRRTL
...
namespace {
struct SynthAnnotationsPass
    : public SynthFIRRTLAnnotationsBase<SynthAnnotationsPass> {
...
LogicalResult SynthAnnotationsPass::applyAnnotation(DictionaryAttr anno,
                                                    ApplyState state) {
  // Lookup the class
...
void SynthAnnotationsPass::runOnOperation() {
...
std::unique_ptr<mlir::Pass> circt::firrtl::createSynthFIRRTLAnnotationsPass(
    bool ignoreUnhandledAnnotations, bool ignoreClasslessAnnotations) {
  auto pass = std::make_unique<SynthAnnotationsPass>();
