...
//===----------------------------------------------------------------------===//
//
// This file declares the operation class for the CHALK IR.
//
//===----------------------------------------------------------------------===//

#ifndef CIRCT_DIALECT_CHALK_OPS_H
#define CIRCT_DIALECT_CHALK_OPS_H

#include "circt/Dialect/CHALK/CHALKDialect.h"
#include "circt/Dialect/CHALK/CHALKOpInterfaces.h"
...
/// Return true if the specified operation is a chalk expression.
...
/// checks for `chalk.constant` and friends, but also looks through subaccesses
...
/// Compute the flow for a Value, \p val, as determined by the CHALK
...
// Type inference adaptor for CHALK operations.
LogicalResult inferReturnTypes(
    MLIRContext *context, Optional<Location> loc, ValueRange operands,
    DictionaryAttr attrs, mlir::RegionRange regions,
    SmallVectorImpl<Type> &results,
    llvm::function_ref<CHALKType(ValueRange, ArrayRef<NamedAttribute>,
                                  Optional<Location>)>
        callback);

// Common type inference functions.
CHALKType inferAddSubResult(CHALKType lhs, CHALKType rhs,
                             Optional<Location> loc);
CHALKType inferBitwiseResult(CHALKType lhs, CHALKType rhs,
                              Optional<Location> loc);
CHALKType inferComparisonResult(CHALKType lhs, CHALKType rhs,
                                 Optional<Location> loc);
CHALKType inferReductionResult(CHALKType arg, Optional<Location> loc);
...
// This is a summary of a CHALK::MemOp. It defines the relevant properties of
// the CHALK memory, and can be constructed by parsing its attributes.
struct FirMemory {
  size_t numReadPorts;
...
namespace llvm {
template <>
struct DenseMapInfo<circt::chalk::FModuleOp> {
  using Operation = mlir::Operation;
  using FModuleOp = circt::chalk::FModuleOp;
...
#endif // CIRCT_DIALECT_CHALK_OPS_H
