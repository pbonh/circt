//===- CHALKTypes.h - CHALK Type System -----------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines type type system for the CHALK Dialect.
//
//===----------------------------------------------------------------------===//

#ifndef CIRCT_DIALECT_CHALK_TYPES_H
#define CIRCT_DIALECT_CHALK_TYPES_H
...
// This is a common base class for all CHALK types.
class CHALKType : public Type {
public:
  ...
  static bool classof(Type type) {
    return llvm::isa<CHALKDialect>(type.getDialect());
  }
  ...
  std::pair<CHALKType, unsigned> getSubTypeByFieldID(unsigned fieldID);
  ...
  CHALKType getFinalTypeByFieldID(unsigned fieldID);
...
/// Returns whether the two types are equivalent. See the CHALK spec for the
/// full definition of type equivalence. This predicate differs from the spec in
/// that it only compares passive types. Because of how the CHALK dialect uses
/// flip types in module ports and aggregates, this definition, unlike the spec,
/// ignores flips.
bool areTypesEquivalent(CHALKType destType, CHALKType srcType);

/// Returns true if two types are weakly equivalent.  See the CHALK spec,
...
bool areTypesWeaklyEquivalent(CHALKType destType, CHALKType srcType,
                              bool destFlip = false, bool srcFlip = false);

mlir::Type getVectorElementType(mlir::Type array);
mlir::Type getPassiveType(mlir::Type anyCHALKType);

//===----------------------------------------------------------------------===//
// Ground Types Without Parameters
//===----------------------------------------------------------------------===//

/// `chalk.Clock` describe wires and ports meant for carrying clock signals.
class ClockType
    : public CHALKType::TypeBase<ClockType, CHALKType, DefaultTypeStorage> {
public:
  using Base::Base;
...
/// `chalk.Reset`.
/// TODO(chalk spec): This is not described in the CHALK spec.
class ResetType
    : public CHALKType::TypeBase<ResetType, CHALKType, DefaultTypeStorage> {
public:
  using Base::Base;
...
/// `chalk.AsyncReset`.
/// TODO(chalk spec): This is not described in the CHALK spec.
class AsyncResetType : public CHALKType::TypeBase<AsyncResetType, CHALKType,
                                                   DefaultTypeStorage> {
public:
  using Base::Base;
...
class WidthQualifiedType
    : public CHALKType::TypeBase<ConcreteType, ParentType,
                                  detail::WidthTypeStorage> {
public:
  using CHALKType::TypeBase<ConcreteType, ParentType,
...
class IntType : public CHALKType {
public:
  using CHALKType::CHALKType;
...
// `chalk.Analog` can be attached to multiple drivers.
class AnalogType : public WidthQualifiedType<AnalogType, CHALKType> {
public:
  using WidthQualifiedType::WidthQualifiedType;
...
/// for CHALK.
class BundleType : public CHALKType::TypeBase<BundleType, CHALKType,
...
public:
  ...
  struct BundleElement {
    ...
    BundleElement(StringAttr name, bool isFlip, CHALKType type)
  ...
  static CHALKType get(ArrayRef<BundleElement> elements, MLIRContext *context);
  ...
  CHALKType getElementType(StringAttr name);
  CHALKType getElementType(StringRef name);

  /// Look up an element type by index.
  CHALKType getElementType(size_t index);
...
class FVectorType : public CHALKType::TypeBase<FVectorType, CHALKType,
                                                detail::VectorTypeStorage> {
public:
  using Base::Base;

  static CHALKType get(CHALKType elementType, unsigned numElements);

  CHALKType getElementType();
...
// unknown bit width.
llvm::Optional<int32_t> getBitWidth(CHALKType type);

// Parse a CHALK type without a leading `!chalk.` dialect tag.
ParseResult parseNestedType(CHALKType &result, AsmParser &parser);

// Print a CHALK type without a leading `!chalk.` dialect tag.
...
template <>
struct DenseMapInfo<circt::chalk::CHALKType> {
  using CHALKType = circt::chalk::CHALKType;
  static CHALKType getEmptyKey() {
    auto pointer = llvm::DenseMapInfo<void *>::getEmptyKey();
    return CHALKType(static_cast<mlir::Type::ImplType *>(pointer));
  }
  static CHALKType getTombstoneKey() {
    auto pointer = llvm::DenseMapInfo<void *>::getTombstoneKey();
    return CHALKType(static_cast<mlir::Type::ImplType *>(pointer));
  }
  static unsigned getHashValue(CHALKType val) { return mlir::hash_value(val); }
  static bool isEqual(CHALKType LHS, CHALKType RHS) { return LHS == RHS; }
};

} // namespace llvm

#endif // CIRCT_DIALECT_CHALK_TYPES_H
