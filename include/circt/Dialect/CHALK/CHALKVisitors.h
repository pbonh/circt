...
//===----------------------------------------------------------------------===//
//
// This file defines visitors that make it easier to work with CHALK IR.
//
//===----------------------------------------------------------------------===//
...
/// ExprVisitor is a visitor for CHALK expression nodes.
...
public:
  ...
  ResultType visitInvalidExpr(Operation *op, ExtraArgs... args) {
    op->emitOpError("unknown CHALK expression");
    abort();
  }
...
/// ExprVisitor is a visitor for CHALK statement nodes.
template <typename ConcreteType, typename ResultType = void,
          typename... ExtraArgs>
class StmtVisitor {
public:
  ...
  ResultType visitInvalidStmt(Operation *op, ExtraArgs... args) {
    op->emitOpError("unknown chalk stmt");
    abort();
  }
...
/// ExprVisitor is a visitor for CHALK declaration nodes.
template <typename ConcreteType, typename ResultType = void,
          typename... ExtraArgs>
class DeclVisitor {
public:
  ...
  ResultType visitInvalidDecl(Operation *op, ExtraArgs... args) {
    op->emitOpError("unknown chalk decl");
    abort();
  }
...
/// CHALKVisitor allows you to visit all of the expr/stmt/decls with one class
/// declaration.
///
/// Clients call dispatchVisitor to invoke the dispatch, and may implement
/// visitInvalidOp() to get notified about non-CHALK dialect nodes and
/// visitUnhandledOp() to get notified about CHALK dialect ops that are not
...
public:
  /// This is the main entrypoint for the CHALKVisitor.
  ...
  /// visitInvalidOp is an override point for non-CHALK dialect operations.
  ResultType visitInvalidOp(Operation *op, ExtraArgs... args) {
    return ResultType();
  }

  /// visitUnhandledOp is an override point for CHALK dialect ops that the
