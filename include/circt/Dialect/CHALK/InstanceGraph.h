...
//===----------------------------------------------------------------------===//
//
// This file defines the CHALK InstanceGraph, which is similar to a CallGraph.
//
//===----------------------------------------------------------------------===//
#ifndef CIRCT_DIALECT_CHALK_INSTANCEGRAPH_H
#define CIRCT_DIALECT_CHALK_INSTANCEGRAPH_H
...
/// to be used as a cached analysis on CHALK circuits.  This class can be used
...
public:
  /// Create a new module graph of a circuit.  This must be called on a CHALK
...
namespace llvm {
template <>
struct GraphTraits<circt::chalk::InstanceGraphNode *> {
  using NodeType = circt::chalk::InstanceGraphNode;
  using NodeRef = NodeType *;

  // Helper for getting the module referenced by the instance op.
  static NodeRef getChild(const circt::chalk::InstanceRecord *record) {
...
template <>
struct GraphTraits<circt::chalk::InstanceGraph *>
    : public GraphTraits<circt::chalk::InstanceGraphNode *> {
  using nodes_iterator = circt::chalk::InstanceGraph::iterator;

  static NodeRef getEntryNode(circt::chalk::InstanceGraph *instanceGraph) {
    return instanceGraph->getTopLevelNode();
  }
  static nodes_iterator
  nodes_begin(circt::chalk::InstanceGraph *instanceGraph) {
    return instanceGraph->begin();
  }
  static nodes_iterator nodes_end(circt::chalk::InstanceGraph *instanceGraph) {
...
template <>
struct GraphTraits<Inverse<circt::chalk::InstanceGraphNode *>> {
  using NodeType = circt::chalk::InstanceGraphNode;
  using NodeRef = NodeType *;

  // Helper for getting the module containing the instance op.
  static NodeRef getParent(const circt::chalk::InstanceRecord *record) {
...
#endif // CIRCT_DIALECT_CHALK_INSTANCEGRAPH_H
